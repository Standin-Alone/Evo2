...
<script>
    $(document).ready(function () {
        $('#filter_region').on('change', function () {
            ...
            var route = "{{route('report.get_filter_province',['reg_name'=>':id'])}}".replace(':id', reg_name);
        ...
        $('#filter_province').on('change', function () {
            ...
            var route = "{{route('report.get_filter_region',['prov_name'=>':id'])}}".replace(':id', prov_name);
            if (prov_name !== '') {
                    ...
                    success: function (regions) {
                        ...
                        $('#filter_region').on('change', function(){
                            if(($('select[name="filter_region"]').val() === '' && $('select[name="filter_province"]').val() !== '')){
                                ...
                                $.ajax({
                                    url: "{{route('report.get_reg_without_prov')}}",
                                    type: "GET",
                                    dataType: "json",
                                    success: function (regions) {
                                        $('select[name="filter_region"]').empty();
                                        ...
                                        $.ajax({
                                            url: "{{route('report.get_prov_without_reg')}}",
                                            type: "GET",
                                            dataType: "json",
                                            success: function (provinces) {
                                                $('select[name="filter_province"]').empty();
