<script>
    // #addBtn_role_and_progs are connected to RfoApprovalModuleController.php
    $(document).on('click', '#addBtn_role_and_progs', function (e) {
        ...
        $('#setup_program_permission_form').ready(function(){
            $('#setup_program_permission_form').validate({
                errorClass: "invalid",
                validClass: "valid",
                rules: {
                    // select_role:{
        ...
        $(document).on('submit', 'form#setup_program_permission_form', function(e){
            ...
            $.ajax({
                ...
                success: function(success_response){
                    Swal.fire({
                        ...
                        $("#setup_program_user_datatable").DataTable().ajax.reload();
                        $("#account_activation_datatable").DataTable().ajax.reload();
                        $('#setup_program_permission_modal').modal('hide');
                    });
                },
            });
        });
    ...
    $('#setup_program_permission_modal').on('hidden.bs.modal', function()
    {
