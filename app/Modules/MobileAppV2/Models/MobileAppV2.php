...
class MobileAppV2 extends Model
{
                            ...
                            if($valid_id_key == 'front'){
                                ...
                                if(!$check_file){
                                    $upload_error_count++;
                                       ...
                                       // update name of file
                                       db::table('voucher_attachments')
                                       ->where('attachment_id',$get_file['front_attachment_id'])
                                       ->where('voucher_id',$voucher_info['voucher_id'])
                                ...
                                if(!$check_file){
                                    ...
                                    // update name of file
                                    db::table('voucher_attachments')
                                    ->where('attachment_id',$get_file['back_attachment_id'])
                                    ->where('voucher_id',$voucher_info['voucher_id'])
                                    ->update(['file_name' => $imageName]);
                                }
                            }
    ...
    public function update_cart(){
        ...
        try{    
            $count = 0;
            $cart = request('cart');
            $removedFromCart = request('removedFromCart');
            $voucher_info = request('voucherInfo');
            $transaction_id = $voucher_info['transaction_id'];
            $reference_no = $voucher_info['reference_no'];
            $fund_id = $voucher_info['fund_id'];
