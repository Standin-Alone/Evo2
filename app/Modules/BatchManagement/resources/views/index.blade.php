...
@section('page-js')    
    ...
    <script type="text/javascript">
       $(document).ready(function() {  
            ...
            function DisbursementApprovalList(kyc_file_id,filterby){
                ...
                var table = $('#DisbursementList-datatable').DataTable({ 
                    ...
                    ajax: ({type:'get',
                            ...
                            data:{kyc_file_id:kyc_file_id,filterby:filterby},
                        }),
                    columns: [
                        {data: 'date_uploaded', name: 'date_uploaded', title:'TRANSACTION DATE'},
            ...
            function getProvinceList(){
                $.ajax({
                    ...
                    success:function(data){                    
                        ...
                        $('.kyc_file_id').append($('<option class="option_prov_code" value="" selected>Select Province</option>'));
                        for(var i=0;i<data.length;i++){                   
                                $('.kyc_file_id').append($('<option>', {class:'option_prov_code',value:data[i].prov_code, text:data[i].prov_name}));
                            }           
                },
                error: function (textStatus, errorThrown) {
                        console.log('Err');
            ...
            $(document).on('change','.kyc_file_id',function(){
                var kyc_file_id = $(this).val();
                var table = $('#DisbursementList-datatable').DataTable();
                table.clear().draw();  
                    ...
                    if (result.isConfirmed) {
                        ...
                        if(kyc_file_id != ""){
                            $.ajax({
                                ...
                                data:{kyc_file_id:kyc_file_id,selected_fund_source:selected_fund_source,_token:_token},
                                success:function(data){ 
                                    Swal.fire({
                                        allowOutsideClick: false,
                                        title:'Approved!',
                        ...
                        if(kyc_file_id != "" && selected_fund_source !=""){
                            if(parseInt(selected_fund_amount) >= parseInt(total_amount)){
                                $.ajax({
                                    type:'post',
                                    url:"{{ route('approve.Disbursement') }}",
                                    data:{kyc_file_id:kyc_file_id,selected_fund_source:selected_fund_source,total_amount:total_amount,_token:_token},
                                    success:function(data){ 
                                        Swal.fire({
                                            allowOutsideClick: false,
                                            title:'Approved!',
                ...
                columns: [                         
                        ...
                        render: function(data,type,row) {
                            @if (session('role_id') == 4)
                                return '<a href="javascript:void(0);" data-kyc_file_id="' + row.kyc_file_id + '" class="kyc_file_idlink"><i class="fas fa-spinner fa-spin '+ data +' pull-left m-r-10" style="display: none;"></i>' + data + '</a>';
                            @elseif (session('role_id') != 4)
                                return data;
                            @endif                            
                        }, title: 'PROVINCE'},
           ...
           function getBeneficiariesforApproval(){
            var table = $('#BeneficiariesforApproval-datatable').DataTable({ 
                ...
                columns: [ 
                        ...
                        render: function(data, type, row) {
                            @if (session('role_id') != 4)
                                return '<a href="javascript:void(0);" data-kyc_file_id="' + row.kyc_file_id + '" class="kyc_file_idlink"><i class="fas fa-spinner fa-spin '+ row.prov_code +' pull-left m-r-10" style="display: none;"></i>' + data + '</a>';
                            @elseif (session('role_id') == 4)
                                return data;
                            @endif  
                        }, title: 'PROVINCE'},
                        ...
                        {data: 'kyc_file_id', name: 'kyc_file_id',  title:'ACTION',
                            render: function(data, type, row) {
                                return '<a href="javascript:;" data-selectedkycid="'+row.kyc_id+'" class="btn btn-xs btn-outline-info btnDisbursementkycDetails"><i class="fas fa-spinner fa-spin '+row.kyc_id+' pull-left m-r-10" style="display: none;"></i><span class="fa fa-check-circle"></span> Approve</a>';
           ...
           $(document).on('click','.kyc_file_idlink',function(){
                ...
                SpinnerShow('kyc_file_id',kyc_file_id);
                $('.selectedbatchall').prop('checked',false); 
                $('.Disbursement_totalselectedamt').val(0);
                $('#Disbursement_selectedamt').val(0);  
                $('#selectedFundSource').val('');
                $('#selectedFundAmount').val(0);
                getFilteredProvinceList(kyc_file_id);
                DisbursementApprovalList(kyc_file_id,0);
                getFundSource();
                $('#ApprovalDisbursementModal').modal('toggle');
                SpinnerHide('kyc_file_id',kyc_file_id);
            });

            function getFilteredProvinceList(kyc_file_id){
                $.ajax({
                    type:'get',
                    url:"{{ route('get.FilteredProvinceList') }}" + '?kyc_file_id=' + kyc_file_id,
                    success:function(data){                    
                        $('.option_prov_code').remove();
                        $('.kyc_file_id').append($('<option class="option_prov_code" value="" selected>Select Province</option>'));
                        for(var i=0;i<data.length;i++){                   
            ...
            <div class="modal-content" style="background-color: #008a8a">
                ...
                <div class="modal-body" style="background-color: #fff">
                    ...
                    <div class="col-md-12 pt-2">
                        <div class="input-group">
                            <!-- <select id="default_DisbursementProvince" class="form-control kyc_file_id" name="DisbursementProvince" data-size="10" data-style="btn-white" value="{{ old('DisbursementProvince') }}">
                            </select> -->
                            @if (session('role_id') == 8)
